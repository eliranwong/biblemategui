...
# Define your tables and their display titles
# Format: "Display Title": "table_name"
TABLE_CONFIG = {
    "Bible People": "exlbp",
    "Bible Locations": "exlbl",
    "Bible Topics": "exlbt",    # This one will be open by default
...
def parse_reference(ref_text: str):
    ...
    # Handles numbers at start of book name (e.g., 1 John)
...
def fetch_data(table_name: str, book_id: int, chapter: int, verse: int):
    ...
    try:
        ...
        with sqlite3.connect(DB_FILE) as conn:
            ...
            # Combine multiple entries if they exist for the same verse
...
def main_page():
    ...
    async def search_action():
        ...
        with results_container:
            ...
            # Run IO-bound DB operations in a separate thread so UI doesn't freeze
            ...
            # --- RENDER EXPANSIONS ---
            # Order: We iterate through TABLE_CONFIG keys to maintain order
            found_any = False
            
            for title, table_name in TABLE_CONFIG.items():
                content = results.get(title)
                
                # Check if this should be open by default
                is_open = (title == "Bible Topics") # Only open Bible Topics
                ...
                with ui.expansion(title, icon='library_books', value=is_open) \
                    ...
                    if content:
                        # Changed from ui.markdown to ui.html to support HTML content
