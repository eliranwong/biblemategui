...
# --- 1. DATA: Bible Structure & Book Names ---
# Mapping IDs to Names for the UI
BOOK_NAMES = {
...
VERSES = {
    ...
    9: {1: 28, 2: 36, 3: 21, 4: 22, 5: 12, 6: 21, 7: 17, 8: 22, 9: 27, 10: 27, 11: 15, 12: 25, 13: 23, 14: 52, 15: 35, 16: 23, 17: 58, 18: 30, 19: 24, 20: 42, 21: 15, 22: 23, 23: 29, 24: 22, 25: 44, 26: 25, 27: 12, 28: 25, 29: 11, 30: 31, 31: 13},
    ...
    11: {1: 53, 2: 46, 3: 28, 4: 34, 5: 18, 6: 38, 7: 51, 8: 66, 9: 28, 10: 29, 11: 43, 12: 33, 13: 34, 14: 31, 15: 34, 16: 34, 17: 24, 18: 46, 19: 21, 20: 43, 21: 29, 22: 53},
    ...
    47: {1: 24, 2: 17, 3: 18, 4: 18, 5: 21, 6: 18, 7: 16, 8: 24, 9: 15, 10: 18, 11: 33, 12: 21, 13: 14},
    ...
    64: {1: 14},
    65: {1: 25},
    66: {1: 20, 2: 29, 3: 22, 4: 11, 5: 14, 6: 17, 7: 17, 8: 13, 9: 21, 10: 11, 11: 19, 12: 17, 13: 18, 14: 20, 15: 8, 16: 21, 17: 18, 18: 24, 19: 21, 20: 15, 21: 27, 22: 21},
}

# --- 2. LOGIC: Generic Draggable Logic ---
...
# --- 3. LOGIC: Bible Selection Dialog ---
class BibleSelector:
    def __init__(self):
        # Create the dialog AND the card container ONCE during initialization
        # Improved styling for maximum screen space utilization:
        # - w-[90vw]: 90% of viewport width
        # - max-w-screen-lg: Limits width on very large screens
        # - h-[90vh]: INCREASED from 80vh to 90vh to maximize vertical space in landscape
        # - flex flex-col: Allows header and content to flow properly
    ...
    def render_chapters(self):
        self.content_area.clear()
        book_name = BOOK_NAMES.get(self.selection['book_id'])
        ...
        with self.content_area:
            ...
            with ui.element('div').classes('w-full flex-1 overflow-y-auto min-h-0'):
                ...
                with ui.row().classes('w-full gap-2 p-2'):
                    chapters = VERSES.get(self.selection['book_id'], {})
    ...
    def render_verses(self):
        ...
        with self.content_area:
            ...
            max_verses = VERSES[self.selection['book_id']][self.selection['chapter']]
...
def main():
    ...
    # Initialize the Bible Selector Logic
    bible_selector = BibleSelector()

    # Generic Layout Swap Logic
    def swap_layout():
        ui.notify('Layout Swapped!', type='info')

    # --- UI Layout ---
    with ui.row().classes('w-full items-center gap-4 p-4'):
        ui.label('Controls:').classes('font-bold')
        toggle_fab1 = ui.switch('Show Layout Button', value=True)
        toggle_fab2 = ui.switch('Show Bible Button', value=True)

    # --- FAB 1: Layout Swap ---
    with ui.column().classes('fixed bottom-6 right-6 z-50 touch-none') as container1:
        make_draggable(container1)
        container1.bind_visibility_from(toggle_fab1, 'value')
        
        ui.button(icon='swap_horiz', on_click=swap_layout) \
            .props('fab-mini color=primary') \
            .tooltip('Swap Layout (Draggable)')

    # --- FAB 2: Bible Selector ---
    # Placed slightly higher (bottom-20) to avoid initial overlap
    with ui.column().classes('fixed bottom-20 right-6 z-50 touch-none') as container2:
        make_draggable(container2)
        container2.bind_visibility_from(toggle_fab2, 'value')
        
        ui.button(icon='menu_book', on_click=bible_selector.open) \
            .props('fab-mini color=secondary') \
            .tooltip('Select Verse (Draggable)')
...
ui.run(title='BibleMate AI - Multi FAB', port=9999)